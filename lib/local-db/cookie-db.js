...
export function mergeOverlayIntoDb(overlay) {
  ...
  // Planning periods: overlay replaces seed when present (including empty = clean slate)
  const periods = Array.isArray(overlay.planning_periods)
    ? overlay.planning_periods
    : seed.planning_periods;
  ...
  return migrateLocalDb({
    ...
    sales_exec_targets: overlay.sales_exec_targets || [],
