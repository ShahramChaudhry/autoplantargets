...
export function getAvailableActions(status, role) {
  ...
  if (role === ROLES.DEMAND_SUPPLY) {
    if (["draft", "b2b_changes_requested", "md_changes_requested"].includes(status)) {
      actions.push({ action: "submit_b2b", label: "Submit for B2B Review" });
    }
    if (status === "b2b_approved") {
      actions.push({ action: "submit_md", label: "Submit to MD" });
    }
    if (status === "md_approved") {
      actions.push({ action: "finalize", label: "Finalize Targets" });
    }
  }

  if (role === ROLES.B2B_DIRECTOR && ["submitted_b2b", "b2b_changes_requested"].includes(status)) {
    actions.push(
  ...
  if (role === ROLES.MANAGING_DIRECTOR && ["submitted_md", "md_changes_requested"].includes(status)) {
    actions.push(
  ...
  if (role === ROLES.NPM && ["finalized", "retail_allocation", "executive_allocation", "reconciliation_failed"].includes(status)) {
...
export function getNextStatus(action, currentStatus) {
  const transitions = {
    ...
    b2b_approve: "b2b_approved",
    b2b_request_changes: "b2b_changes_requested",
    submit_md: "submitted_md",
