...
export async function calculateReconciliation(supabase, planningPeriodId) {
  ...
  // Prefer sales_exec_targets (Exec × Model leaves); fall back to legacy executive_allocations
  let executiveSum = 0;
  const execTargetsResult = await supabase
    .from("sales_exec_targets")
    .select("target_units")
    .eq("planning_period_id", planningPeriodId);

  if (!execTargetsResult.error) {
    executiveSum = (execTargetsResult.data || []).reduce(
      (sum, e) => sum + (Number(e.target_units) || 0),
      0
    );
  }
