...
export async function getActivePlan(planSlugParam) {
  ...
  // Prefer a plan in (or ready for) sales-office allocation over newest draft
  const retailReady =
    periods.find((p) => p.status === "finalized") ||
    periods.find((p) =>
      ["retail_allocation", "executive_allocation", "reconciliation_failed"].includes(p.status)
    );
  if (retailReady) return retailReady;

  return periods[0] || null;
}

/** Plan that belongs in a role's approval/review queue (falls back to first visible). */
export async function getQueuePlan(role, planSlugParam) {
  const periods = await getPlanningPeriods();

  if (planSlugParam) {
    const matched = findPlanBySlug(periods, planSlugParam);
    if (!matched) return null;
    // Allow deep-link to any period so history remains viewable
    return matched;
  }

  if (role === ROLES.MANAGING_DIRECTOR) {
    return (
      periods.find((p) => isAwaitingMDApproval(p.status)) ||
      periods.find((p) => isVisibleToMD(p.status)) ||
      null
    );
  }
  ...
  if (role === ROLES.B2B_DIRECTOR) {
    return (
      periods.find((p) => isAwaitingB2BReview(p.status)) ||
      periods.find((p) => isVisibleToB2B(p.status)) ||
      null
    );
  }
