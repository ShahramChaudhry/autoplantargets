...
export async function fetchRetailAllocationTotals(supabase, planningPeriodId) {
  ...
  // D&S totals: model-level rows (preferred), else brand-level Retail pool for progress only
  const modelLevel = (retailTargets || []).filter((t) => !t.sales_office && t.model);
  const brandLevel = (retailTargets || []).filter((t) => !t.sales_office && !t.model);
