import { getArticleCodesForModel } from "./master-data.js";
...
/**
 * Sum leaf office allocations for one model (articles if present, else model×office).
 */
export function sumModelOfficeLeaves(targets, brand, salesGroup, model) {
  ...
  const hasArticleLeaves = rows.some((t) => t.article_code);
  const articleCodes = getArticleCodesForModel(brand, model);

  if (articleCodes.length > 0 || hasArticleLeaves) {
    return rows
