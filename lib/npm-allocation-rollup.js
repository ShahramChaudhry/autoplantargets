...
/** Sum leaf office units (model or article) for brand × sales group × office. */
export function sumOfficeGroupUnits(targets, brand, salesGroup, officeName) {
  return (targets || [])
    .filter(
      (t) =>
        t.brand === brand &&
...
/** D&S brand total for one sales group (header model rows only). */
export function dsBrandGroupTotal(targets, brand, salesGroup) {
  return (targets || [])
    .filter(
      (t) =>
        t.brand === brand &&
...
function dsModelsForBrandGroup(targets, brand, salesGroup) {
  return (targets || [])
    .filter(
      (t) =>
        t.brand === brand &&
        ...
        !t.sales_office &&
        !t.article_code &&
        (Number(t.target_units) || 0) > 0
    )
    .map((t) => ({ model: t.model, units: Number(t.target_units) || 0 }));
}

function existingModelMixForOffice(targets, brand, salesGroup, officeName) {
  const byModel = {};
  for (const t of targets || []) {
    ...
    byModel[t.model] = (byModel[t.model] || 0) + (Number(t.target_units) || 0);
  }
  return byModel;
}

/** Split an office total across models using largest-remainder. */
function allocateByWeights(total, weightedModels) {
  ...
  if (total <= 0 || weightSum <= 0 || weightedModels.length === 0) {
    return weightedModels.map((m) => ({ model: m.model, units: 0 }));
  }
  ...
  return parts.map((p) => ({ model: p.model, units: p.units }));
}

/**
 * Expand office × sales-group totals into model × office leaves.
 * Prefers scaling the existing model mix per office; otherwise uses D&S model mix.
 */
export function expandOfficeAllocationsToModelLeaves({
  brand,
  officeAllocations,
  targets,
}) {
  ...
  for (const row of officeAllocations || []) {
    if (!row.sales_group || !row.sales_office) continue;
    if (!byGroup.has(row.sales_group)) byGroup.set(row.sales_group, []);
    byGroup.get(row.sales_group).push(row);
  }

  for (const [salesGroup, officeRows] of byGroup) {
    const dsModels = dsModelsForBrandGroup(targets, brand, salesGroup);
    if (dsModels.length === 0) continue;

    for (const officeRow of officeRows) {
      const officeName = officeRow.sales_office;
      const officeTotal = Number(officeRow.target_units) || 0;
      const existing = existingModelMixForOffice(
        targets,
        brand,
        salesGroup,
        officeName
      ...
      const weighted =
        existingTotal > 0
          ? dsModels.map((m) => ({
              model: m.model,
              weight: existing[m.model] || 0,
            }))
          : dsModels.map((m) => ({ model: m.model, weight: m.units }));

      // If existing mix only covers some models, fall back to D&S weights when all zero
